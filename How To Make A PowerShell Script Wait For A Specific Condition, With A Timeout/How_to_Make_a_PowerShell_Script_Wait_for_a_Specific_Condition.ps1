...
while (-not (Test-Path -Path 'C:\Snip\test.txt')) {
...
$timer = [Diagnostics.Stopwatch]::StartNew()
while (($timer.Elapsed.TotalSeconds -lt 10) -and (-not (Test-Path -Path 'C:\Snip\test.txt'))) {
	Start-Sleep -Seconds 1
...
Wait-Action -Condition {Test-Path -Path 'C:\Snip\test.txt'} -Timeout 5 -RetryInterval 1

Wait-Action -Condition {Test-Connection -ComputerName 'offline' -Count 1 -Quiet} -Timeout 10 -RetryInterval 2