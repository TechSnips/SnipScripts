function New-CustomEC2Instance {
	...
	## Create the VPC using a user-defined CIDR block (network)
	...
	## Create the internet gateway that will act as a router for Internet traffic for our VPC
	...
	## Create a new route table on our VPC so we can add a default route to it
	...
	## Create the subnet and add it to the route table
